...
test_that("initial poc",{
  ...
  expect_equal(prepare_factors_dt(train$some_new, test$some_new, rare_count = 5L),
               c("rare", "rare", "rare", "rare", "rare", "b", "b", "rare", "b", 
                 "b", "b", NA, NA, "other", "rare", "rare", "other", "rare", "b", 
                 "b", "rare", "b", "b", "b", NA, NA)
  ) 
  
  expect_equal(prepare_factors_dt(c(NA, NA, NA, NA, NA, "a"), 
                                    c("b",NA, "a")),
...
test_that("check that order of integers doesn't change",{
  ...
  expect_equal(prepare_factors_dt(train, test, output_type = "character", rare_count = 5L),
               c("a", "a", "a", "a", "a", "rare", "rare", "a", "a", "a", "a", "a")
  )
  expect_equal(prepare_factors_dt(train, test, output_type = "factor", rare_count = 5L),
               structure(c(1L, 1L, 1L, 1L, 1L, 2L, 2L, 1L, 1L, 1L, 1L, 1L), 
                         .Label = c("a", "rare", "other"), class = c("ordered", "factor"))
  )
  expect_equal(prepare_factors_dt(train, test, output_type = "integer", rare_count = 5L),
  ...
  expect_equal(prepare_factors_dt(c("a", "a", "a")),
               c("rare", "rare", "rare")
  )
  expect_equal(prepare_factors_dt(c("a", "a", "a", "a", "a")),
               c("a", "a", "a", "a", "a")
  )
  expect_equal(prepare_factors_dt("a", NA_character_),
               c("rare", NA)
  )
...
test_that("Input checks",{
  ...
  expect_error(prepare_factors_dt(TRUE),
               "`train` must be a character.",
               fixed = TRUE)
  expect_error(prepare_factors_dt("a", TRUE),
               '`test` must be a character when it is provided.',
               fixed = TRUE)
  expect_error(prepare_factors_dt("a", rare_count = TRUE),
               '`rare_count` must be an integer.',
               fixed = TRUE)
  expect_error(prepare_factors_dt("a", rare_level = TRUE),
               '`rare_level` must be a character.',
               fixed = TRUE)
  expect_error(prepare_factors_dt("a", new_level = TRUE),
               '`new_level` must be a character.',
               fixed = TRUE)
  expect_error(prepare_factors_dt("a", ignore_na = "hat"),
               '`ignore_na` must be a logical.',
               fixed = TRUE)
  expect_error(prepare_factors_dt("a", encoding = "hat"),
               '`encoding` is not implemented at the moment.',
               fixed = TRUE)
  expect_error(prepare_factors_dt("a", return_mapping = "bla"),
               '`return_mapping` must be a logical.',
               fixed = TRUE)
  expect_warning(prepare_factors_dt("other"),
                 '`new_level` is already a level in `train`.',
                 fixed = TRUE)
  expect_warning(prepare_factors_dt("a", "other"),
                 '`new_level` is already a level in `test`.',
                 fixed = TRUE)
  expect_warning(prepare_factors_dt("rare", "a"),
                 '`rare_level` is already a level in `train`.',
                 fixed = TRUE)
  expect_warning(prepare_factors_dt("a", "rare"),
